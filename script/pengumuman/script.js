...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (fileParam) {
    ...
    setTimeout(() => {
      const activeItem = document.querySelector(`.pengumuman-item[data-filename="${fileParam}"]`);
      if (activeItem) {
        document.querySelectorAll('.pengumuman-item').forEach(i => i.classList.remove('active'));
...
async function loadMarkdownContent(filename) {
  ...
  if (!filename || !filename.toLowerCase().endsWith('.md')) {
    console.error('[Pengumuman] Invalid file extension:', filename);
  ...
  if (sanitizedFilename !== filename) {
    console.warn('[Pengumuman] Filename contained invalid characters, sanitized:', filename, '->', sanitizedFilename);
  }
  ...
  try {
    ...
    console.log('[Pengumuman] Fetching:', filePath);
    ...
    if (trimmedContent.startsWith('<!doctype') || trimmedContent.startsWith('<html')) {
      console.error('[Pengumuman] File is HTML, not markdown:', sanitizedFilename);
...
// Lightbox untuk gambar di konten Pengumuman
function initContentLightbox() {
  ...
  // Delegate click pada img-wrapper di dalam pengumuman-body
  const pengumumanBody = contentEl.querySelector('.pengumuman-body');
  if (!pengumumanBody) return;

  pengumumanBody.addEventListener('click', (e) => {
